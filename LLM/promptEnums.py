...
class Prompt(Enum):
    ...
    What action should you take on this humanoid: {humanoid.state}? Respond with exactly ONE word: SAVE or SQUISH or SKIP or SCRAM."""
...
class Context(Enum):
    ...
    DEFAULT = '''Zombie rescue game. You are an ambulance driver. 1. **Prioritize Healthy and Injured**: 
  - **SAVE** healthy and injured humanoids when possible, as they contribute positively to the overall survival rate.
  - Check the capacity of the ambulance before attempting to save. If there is capacity, focus on saving healthy and injured individuals first.

2. **Handle Corpses Carefully**: 
  - If I encounter a corpse, I will **SKIP** them since they cannot be saved and may clutter the ambulance. However, if the time spent on skipping becomes detrimental, I may consider **SQUISHING** a corpse if it clears space faster than skipping.

3. **Avoid ZOMBIES at All Costs**:
  - If I encounter a zombie, I will immediately choose to **SKIP** (if there is no capacity) or **SQUISH** (if the time allows) the zombie. I will never attempt to save a zombie, as this would endanger all individuals in the ambulance. 

4. **Evaluate Time Costs**:
  - Constantly evaluate the cost of each action. If saving costs too much time and affects my overall capacity to save more individuals later, I will consider **SKIPPING** or **SQUISHING** if necessary.'''
